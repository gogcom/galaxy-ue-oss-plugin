...
namespace
{
	...
	bool CheckFriendListName(const FString& InListName)
	{
		...
		{
			UE_LOG_ONLINE_FRIEND(Warning, TEXT("Only a default list name '%s' is supported: listName='%s'"),
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::ReadFriendsList(%s)"), *InListName);
	...
	if (err)
	{
		...
		UE_LOG_ONLINE_FRIEND(Warning, TEXT("Failed to request friends list: listname=%s; %s"), *InListName, *errorMessage);
...
{
	UE_LOG_ONLINE_FRIEND(Warning, TEXT("FOnlineFriendsGOG::DeleteFriendsList() is not supported by GOG platform"));
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::SendInvite(%s, %s)"), *InListName, *InFriendId.ToString());
	...
	{
		const auto errorMessage = FString::Printf(TEXT("%s: %s"), UTF8_TO_TCHAR(err->GetName()), UTF8_TO_TCHAR(err->GetMsg()));
		UE_LOG_ONLINE_FRIEND(Warning, TEXT("Failed to send friend invitation: listname=%s, friendID=%s; %s"), *InListName, *InFriendId.ToString(), *errorMessage);
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::AcceptInvite(%s, %s)"), *InListName, *InFriendId.ToString());
	...
	{
		const auto errorMessage = FString::Printf(TEXT("%s: %s"), UTF8_TO_TCHAR(err->GetName()), UTF8_TO_TCHAR(err->GetMsg()));
		UE_LOG_ONLINE_FRIEND(Warning, TEXT("Failed to accept friend invitation: listname=%s, friendID=%s; %s"), *InListName, *InFriendId.ToString(), *errorMessage);
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::RejectInvite(%s, %s)"), *InListName, *InFriendId.ToString());
	...
	{
		const auto errorMessage = FString::Printf(TEXT("%s: %s"), UTF8_TO_TCHAR(err->GetName()), UTF8_TO_TCHAR(err->GetMsg()));
		UE_LOG_ONLINE_FRIEND(Warning, TEXT("Failed to reject friend invitation: listname=%s, friendID=%s; %s"), *InListName, *InFriendId.ToString(), *errorMessage);
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::DeleteFriend(%s, %s)"), *InListName, *InFriendId.ToString());
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::GetFriendsList(%s)"), *InListName);
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::GetFriend(%s, %s)"), *InListName, *InFriendId.ToString());
...
TSharedRef<FOnlineFriend>* const FOnlineFriendsGOG::FindFriend(const FUniqueNetId& InFriendId, const FString& InListName)
{
	...
	if (!cachedFriend)
	{
		UE_LOG_ONLINE_FRIEND(Display, TEXT("Friend not found on the list: listName='%s', friendID='%s'"), *InListName, *InFriendId.ToString());
		return{};
	}
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::IsFriend(%s, %s)"), *InListName, *InFriendId.ToString());
...
{
	UE_LOG_ONLINE_FRIEND(Warning, TEXT("FOnlineFriendsGOG::QueryRecentPlayers() is not supported by GOG platform"));
...
{
	UE_LOG_ONLINE_FRIEND(Warning, TEXT("FOnlineFriendsGOG::GetRecentPlayers() is not supported by GOG platform"));
	return false;
}

bool FOnlineFriendsGOG::BlockPlayer(int32 InLocalUserNum, const FUniqueNetId& InPlayerId)
{
	UE_LOG_ONLINE_FRIEND(Warning, TEXT("FOnlineFriendsGOG::BlockPlayer() is not supported by GOG platform"));
...
{
	UE_LOG_ONLINE_FRIEND(Warning, TEXT("FOnlineFriendsGOG::UnblockPlayer() is not supported by GOG platform"));
...
{
	UE_LOG_ONLINE_FRIEND(Warning, TEXT("FOnlineFriendsGOG::QueryBlockedPlayers() is not supported by GOG platform"));
...
{
	UE_LOG_ONLINE_FRIEND(Warning, TEXT("FOnlineFriendsGOG::GetBlockedPlayers() is not supported by GOG platform"));
	return false;
}
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::OnFriendListRetrieveSuccess()"));

	auto friendCount = galaxy::api::Friends()->GetFriendCount();
	auto err = galaxy::api::GetError();
	if (err)
	{
		UE_LOG_ONLINE_FRIEND(Error, TEXT("Failed to get players friend count: %s: %s"), UTF8_TO_TCHAR(err->GetName()), UTF8_TO_TCHAR(err->GetMsg()));
		return;
	}
	...
	for (decltype(friendCount) friendIdx{0}; friendIdx < friendCount; ++friendIdx)
	{
		...
		if (err)
		{
			UE_LOG_ONLINE_FRIEND(Error, TEXT("Failed to get player friends: %s: %s"), UTF8_TO_TCHAR(err->GetName()), UTF8_TO_TCHAR(err->GetMsg()));
			continue;
		}

		if (!UserInfoUtils::IsUserInfoAvailable(friendID))
		{
			UE_LOG_ONLINE_FRIEND(Error, TEXT("Friend retrieved yet no user infromation available: friendID='%s'"), *friendID.ToString());
			continue;
		}
...
{
	UE_LOG_ONLINE_FRIEND(Warning, TEXT("FOnlineFriendsGOG::OnFriendListRetrieveFailure()"));
}

void FOnlineFriendsGOG::OnFriendInvitationReceived(galaxy::api::GalaxyID InFriendID, uint32_t /*InSendTime*/)
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::OnFriendInvitationReceived()"));
...
{
	UE_LOG_ONLINE_FRIEND(Display, TEXT("FOnlineFriendsGOG::OnFriendAdded()"));
