...
{
	UE_LOG_ONLINE_SESSION(Display, TEXT("FUpdateLobbyListener::OnLobbyDataUpdateSuccess: lobbyID=%llu"), InLobbyID.ToUint64());
...
{
	UE_LOG_ONLINE_SESSION(Warning, TEXT("FUpdateLobbyListener::OnLobbyDataUpdateFailure: lobbyID=%llu"), InLobbyID.ToUint64());

	switch (InFailureReason)
	{
		case galaxy::api::ILobbyDataUpdateListener::FAILURE_REASON_LOBBY_DOES_NOT_EXIST:
		{
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Specified lobby does not exists anymore: lobbyID=%llu"), InLobbyID.ToUint64());
			break;
		}
		case galaxy::api::ILobbyDataUpdateListener::FAILURE_REASON_UNDEFINED:
		default:
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Unknown faliure during lobby data update: lobbyID=%llu"), InLobbyID.ToUint64());
			break;
	}
